...
export default function ConfirmacaoPix({ onConfirmar, onVoltar }: ConfirmacaoPixProps) {
    const [copiado, setCopiado] = useState(false);

    const copiarChave = async () => {
        try {
            // Método moderno — só funciona em HTTPS ou localhost
            await navigator.clipboard.writeText(CHAVE_PIX);
        } catch {
            // Método alternativo — funciona em qualquer contexto,
            // inclusive http:// via IP local (celular na mesma rede)
